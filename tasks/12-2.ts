...
const buildPath = (start: string, graph: Graph, path: string[]): void => {
  ...
  for (const point of graph[start]) {
    if (path.indexOf(point) === -1 || isUpperCase(point) || (point !== 'start' && !hasLowerCaseDupes(path))) {
...
const hasLowerCaseDupes = (path: string[]) => {
  for (const point of path) {
    if (!isUpperCase(point) && point !== 'start' && path.indexOf(point) !== path.lastIndexOf(point)) {
      return true;
    }
  }
